...
from highway_env.vehicle.behavior import IDMVehicle, LinearVehicle, AggressiveVehicle, AggressiveCar, VeryAggressiveCar
...
class VehicleGraphics(object):
    ...
    def display(cls, vehicle, surface, transparent=False, offscreen=False):
        ...
        sur = pygame.Surface((surface.pix(v.LENGTH + 200), surface.pix(v.LENGTH + 200)), pygame.SRCALPHA)
        if (isinstance(v, MDPVehicle)):
            pygame.draw.circle(sur,(255,0,0),(100, 100),surface.pix(12),3)
        pygame.draw.rect(s, cls.get_color(v, transparent), rect, 0)
        pygame.draw.rect(s, cls.BLACK, rect, 1)
        # draw a circle of some radius around the vehicle
        ...
        text = font.render(str(v).split()[1].split(":")[0], True, (255,255,0), (0,0,255)) 
        ...
        textRect.center = surface.pos2pix(v.position[0] - v.LENGTH / 2, v.position[1] - v.LENGTH / 2) 
        surface.blit(sr, (surface.pos2pix(v.position[0] - v.LENGTH / 2, v.position[1] - v.LENGTH / 2)))
        surface.blit(sur, (surface.pos2pix(v.position[0]-17-v.WIDTH/2, v.position[1]-17-v.WIDTH/2)))
